...
module.nodes = {
	...
	_init: function( node, parentModel ) {
		...
		if("undefined" == typeof(parentModel.childEvents)){
			parentModel.childEvents = [];
		}
		...
		switch ( node.nodeName ) {
			case "DIV": case "UL":case "DL":case "FIELDSET":
			case "FORM":case "LI":case "DD":
			case "TABLE":case "TR":case 'P': case 'TD':
			case "ARTICLE": case "SECTION": case "FIGURE":
				...
				if ( model_name && model_name !='' ) {
					...
					if("object" == typeof this.models.fns[model_name]){
					...
					if("undefined" == typeof(parentModel.childModels[model_name])){
						parentModel.childModels[model_name] = [];
					}
					parentModel.childModels[model_name].push( node );
					
					if("undefined" == typeof(this.models[model_name])){
						this.models[model_name] = [];
					}
					this.models[model_name].push( node );
			...
			case "H1": case "H2": case "H3": case "H4":case "I":
				...
				if ( event_name ) {
					...
					if("undefined" == typeof(parentModel.childEvents[event_name])){
						parentModel.childEvents[event_name] = [];
					}
					parentModel.childEvents[event_name].push( node );
					
					if("undefined" == typeof(this.events[event_name])){
						this.events[event_name] = [];
					}
					this.events[event_name].push( node );
				}
				break;
			case "HEAD": case "BODY":
	...
	_onload: {
		execute: function() {
			var l = this.queue.length,i;
			for ( i = 0; i < l; i ++ ) {
				if(this.queue[i] && $(this.queue[i]).length > 0){
					this.queue[i]["load"]();
