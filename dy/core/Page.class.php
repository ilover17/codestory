...
{//类定义开始
    ...
    {   
        $this->config    =  array('header'=>L('PUBLIC_T_RECORD'),'prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后页');
    ...
    public function show($isArray=false){
        ...
        if( $this->nowPage <= $halfRoll || $this->totalPages <= $this->rollPage ){
            ...
            $prePage = "<a href='".$url."&".C('VAR_PAGE')."=$preRow' >".L('PUBLIC_UP_PAGE',array('page'=>$this->rollPage))."</a>";
		...
		if( ($this->totalPages-$this->nowPage) < $halfRoll || $this->totalPages <= $this->rollPage ){
            ...
            $nextPage = "<a href='".$url."&".C('VAR_PAGE')."=$nextRow' >".L('PUBLIC_DOWN_PAGE',array('page'=>$this->rollPage))."</a>";
	...
	public function wapShow($isArray=false){
        ...
        if( $this->nowPage <= $halfRoll || $this->totalPages <= $this->rollPage ){
            ...
            $preRow =  $this->nowPage-$this->rollPage;
            $prePage = "<a href='".$url."&".C('VAR_PAGE')."=$preRow' class='pre'>".L('PUBLIC_UP_PAGE',array('page'=>$this->rollPage))."</a>";
            $theFirst = "<a href='".$url."&".C('VAR_PAGE')."=1' >1..</a>";
        }
		...
		if( ($this->totalPages-$this->nowPage) < $halfRoll || $this->totalPages <= $this->rollPage ){
            ...
            $nextRow = $this->nowPage+$this->rollPage;
            $theEndRow = $this->totalPages;
            $nextPage = "<a href='".$url."&".C('VAR_PAGE')."=$nextRow' class='next'>".L('PUBLIC_DOWN_PAGE',array('page'=>$this->rollPage))."</a>";
            $theEnd = "<a href='".$url."&".C('VAR_PAGE')."=$theEndRow' >..{$theEndRow}</a>";
        }
		...
		if( $this->totalPages > 1 ){
			...
			$pageStr = '<form method="post" action="'.$nowUrl.'"><span>'.$upPage.'&nbsp;'.$downPage.'&nbsp;'.$this->nowPage.'/'.$this->totalPages.L('PUBLIC_PAGE').'</span>';
			$pageStr .= '<input type="text" style="margin-left:8px;width:40px"  name="p" id="p" value="'.$this->nowPage.'">';
			$pageStr .= '<input type="submit" value="'.L("PUBLIC_JUMP_TO").'"/>
				</form>';
		}
