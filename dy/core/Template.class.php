...
{//类定义开始
    ...
    public function loadTemplate ($tmplTemplateFile='')
    {
        ...
        if(!is_file($tmplTemplateFile)){
            ...
            if(!is_file($tmplTemplateFile))
                throw_exception(L('模板不存在'));
        }
        $this->templateFile    =  $tmplTemplateFile;

        //根据模版文件名定位缓存文件
        $tmplCacheFile = $this->config['cache_path'].'/'.qm_md5($tmplTemplateFile).$this->config['cache_suffix'];
        $tmplContent = '';
        // 检查Cache文件是否需要更新
    ...
    public function parseInclude($tmplPublicName){
		...
		//qimingdao修改: 2009-5-28 可以导入风格模板目录下的模板文件，写的比较死，以后再优化
