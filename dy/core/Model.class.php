...
qmdefine('HAS_ONE',1);
qmdefine('BELONGS_TO',2);
qmdefine('HAS_MANY',3);
qmdefine('MANY_TO_MANY',4);
/**
 +------------------------------------------------------------------------------
 * Model模型类
...
class Model extends Qiming
{
    ...
    public function switchModel($type,$vars=array()) {
        ...
        if(!class_exists($class))
            throw_exception($class.L('model 不存在'));
        // 实例化扩展模型
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),array('field','table','where','order','limit','page','having','group','lock','distinct'),true)) {
            ...
            throw_exception(__CLASS__.':'.$method.L('方法不存在'));
            return;
        }
    }
    // 回调方法 初始化模型
    public function _initialize() {}
    public function getSourceInfo($ids = '') {}
     ...
     protected function _facade($data) {
        ...
        if(!empty($this->fields)) {
            foreach ($data as $key=>$val){
                ...
                if(C('DB_FIELDTYPE_CHECK') && is_scalar($val)) {
                    // 字段类型检查
    ...
    public function getFieldAsArray($field='*'){
    	$options =  $this->_parseOptions(array());
    	$resultSet = $this->db->select($options,'getFieldAsArray',$field);
